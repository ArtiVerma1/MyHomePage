            ...
            observable.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread()).subscribe(object : Observer<String?> {
                    ...
                    override fun onNext(s: String) {
                        ...
                        when (radius) {
                            "0" -> {
                                round = RequestOptions().placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher).dontTransform().diskCacheStrategy(
                                    DiskCacheStrategy.ALL)
                            }
                            else -> {
                                round = RequestOptions().placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher).transform(
